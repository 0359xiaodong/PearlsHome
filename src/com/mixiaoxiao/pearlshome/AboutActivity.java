...
public class AboutActivity extends FragmentActivity {
	...
	private MxxNotifyingScrollView view_app, view_me;
	int last_t = 0;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		getActionBar().setTitle(MxxTextUtil.getTypefaceSpannableString(this, "About",MxxTextUtil.Roboto_Light, false));
...
//		scaleImageView = (MxxScaleImageView) findViewById(R.id.about_mxxScaleImageView1);
		...
		ArrayList<View> views = new ArrayList<View>(2);
		 view_app = (MxxNotifyingScrollView) LayoutInflater.from(this).inflate(R.layout.about_layout_app, null);
		 view_me = (MxxNotifyingScrollView) LayoutInflater.from(this).inflate(R.layout.about_layout_author, null);;
//		 ((TextView)view_app.findViewById(R.id.about_textview)).setTypeface(typeface);
//		 ((TextView)view_me.findViewById(R.id.about_textview)).setTypeface(typeface);
		
		
		views.add(view_app);
		views.add(view_me);
		mViewPager.setAdapter(new ViewpagerAdapter(views,new String[]{ getString(R.string.app_name),"author"}));
...
//				Log.e("TEMP", String.format("l:%d t:%d oldl:%d oldt:%d dy:%d", l, t, oldl, oldt, deltaY));
		...
		view_app.setOnScrollChangedListener(onScrollChangedListener);
...
//		layoutParams.height = MxxUiUtil.dip2px(this, 48) + config.getPixelInsetTop(true);
		...
		view_app.setPadding(0,  config.getPixelInsetTop(true) + top_margin, config.getPixelInsetRight(), config.getPixelInsetBottom());
		view_app.requestLayout();
